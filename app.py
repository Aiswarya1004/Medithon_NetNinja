...
def send_reminder(patient_id, medication, dosage, contact):
    ...
    # print(f"Reminder: Patient {patient_id} should take {dosage} of {medication} with contact {contact}")
    # Example: send_telegram_message(contact, f"Reminder: Please take {dosage} of {medication}")
    send_telegram_message(contact, f"Reminder: Please take {dosage} of {medication}")
...
def book_appointment():
    ...
    with connection.cursor() as cursor:
        cursor.execute("""
            INSERT INTO Appointment (name, email, mobile, date, note)
            VALUES (%s, %s, %s, %s, %s)
        """, (data['name'], data['email'], data['mobile'], data['date'], data['note']))
    connection.commit()
    connection.close()
